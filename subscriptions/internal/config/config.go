...
func LoadConfig() *Config {
	if err := godotenv.Load(); err != nil {
		log.Println("Note: .env file not found or could not be loaded in subscriptions service, using system environment variables")
	}
	...
	dbName := getEnv("DB_NAME", "webhook_subscriptions")
...
func ConnectDB(optionalCfg ...*Config) (*gorm.DB, error) {
			...
			if sqlDB, err := DB.DB(); err == nil {
				if pingErr := sqlDB.Ping(); pingErr == nil {
					...
					if err := DB.AutoMigrate(
						...
						return nil, fmt.Errorf("failed to auto-migrate subscriptions database: %w", err)
					}
					DB.Exec("SET FOREIGN_KEY_CHECKS = 1;")
	...
	return nil, fmt.Errorf("failed to connect to subscriptions database after %d attempts: %w", maxRetries, err)
}
