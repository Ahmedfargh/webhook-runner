...
func LoadConfig() *Config {
	if err := godotenv.Load(); err != nil {
		log.Println("Note: .env file not found or could not be loaded in webhook-runner service, using system environment variables")
	}

	port := getEnv("GRPC_PORT", "50053")
	authToken := getEnv("AUTH_TOKEN", "4f7f956f34bcfa0c9a55aff6b98c4e1d87e1da6d0d33f5021b5937123d7330c1")

	allowedServicesRaw := getEnv("ALLOWED_SERVICES", "api-gateway,webhook-runner")
	...
	dbName := getEnv("DB_NAME", "webhook_runner")
	...
	kafkaGroupID := getEnv("KAFKA_GROUP_ID", "webhook-runner-group")
...
func ConnectDB(optionalCfg ...*Config) (*gorm.DB, error) {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
			...
			if sqlDB, err := DB.DB(); err == nil {
				if pingErr := sqlDB.Ping(); pingErr == nil {
					...
					if err := DB.AutoMigrate(
						...
						return nil, fmt.Errorf("failed to auto-migrate webhook runner database: %w", err)
					}
					DB.Exec("SET FOREIGN_KEY_CHECKS = 1;")
		...
		log.Printf("Waiting for webhook-runner database at %s:%s (attempt %d/%d): %v\n",
			cfg.DBHost, cfg.DBPort, attempt, maxRetries, err)
		time.Sleep(2 * time.Second)
	}

	return nil, fmt.Errorf("failed to connect to webhook-runner database after %d attempts: %w", maxRetries, err)
}
